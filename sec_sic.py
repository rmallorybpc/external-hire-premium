...
def parse_neo_rows(sct, ceo_name_hint):
    ...
    if sct is None or sct.empty:
        return []
    df = sct.copy()
    # Flatten column headers into a synthetic first row so we catch the header
    # whether read_html put it in df.columns OR left it in the table body.
    col_header = [" ".join(str(x) for x in (c if isinstance(c, tuple) else (c,)))
                  for c in df.columns]
    arr = np.vstack([np.array(col_header, dtype=object), df.astype(str).values])
    nrows, ncols = arr.shape
    # locate the header row: the one containing both 'salary' and 'total'
    header_row = None
    for i in range(min(nrows, 10)):
        rt = " ".join(str(c) for c in arr[i]).lower()
        if "salary" in rt and "total" in rt:
            header_row = i
            break
    if header_row is None:
        return []
    # map columns by position from that header row ('total' last wins = SCT total)
    col = {}
    for j in range(ncols):
        h = str(arr[header_row][j]).lower()
        if "year" in h and "year" not in col: col["year"] = j
        if "salary" in h and "salary" not in col: col["salary"] = j
        if "total" in h: col["total"] = j
    rows = []
    last_name = None
    hint = (ceo_name_hint or "").split()[-1].lower() if ceo_name_hint else ""
    for i in range(header_row + 1, nrows):
        nm = str(arr[i][0]).strip()
        if nm and nm.lower() != "nan":
            last_name = nm
        blob = " ".join(str(c) for c in arr[i]).lower()
        is_ceo = ("chief executive" in blob or re.search(r"\bceo\b", blob) is not None
                  or (hint and hint in (last_name or "").lower()))
        rows.append({
            "name": last_name,
            "year": str(arr[i][col["year"]]).strip() if "year" in col else "",
            "salary": _money(arr[i][col["salary"]]) if "salary" in col else None,
            "total": _money(arr[i][col["total"]]) if "total" in col else None,
            "is_ceo": bool(is_ceo),
        })
    return rows

def _money(x):
    """Parse a dollar cell, dropping footnote markers like '(6)' and '$'/commas."""
    s = str(x).split("(")[0]
    s = re.sub(r"[^0-9.]", "", s)
...
def run(panel_path="edgar_comp_panel.csv"):
    ...
    for _, p in panel.iterrows():
        ...
        for role, who, pyr, fy in [("succ", p["successor"], p["succ_proxy_year"], p["succ_first_full_fy"]),
            ...
            rec[f"{role}_status"] = ("ok" if (target and target.get("total") is not None)
                                     else "total-not-extracted" if target
                                     else "ceo-row-not-found")
            for n in neo:
