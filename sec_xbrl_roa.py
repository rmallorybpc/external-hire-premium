...
def prices_yf(ticker, start, end):
    ...
    if df is None or len(df) == 0:
        return None
    close = df["Close"]
    if hasattr(close, "columns"):      # current yfinance may return multi-level columns
        close = close.iloc[:, 0]
...
# ---- main ------------------------------------------------------------
def run(panel_path="wrds_panel_gvkeys.csv", overrides_path="ticker_overrides.csv"):
    import os
    if overrides_path and not os.path.exists(overrides_path):
        overrides_path = None
    ...
    for _, p in panel.iterrows():
        ...
        full_start = appt - pd.DateOffset(years=YRS_BEFORE)
        full_end   = appt + pd.DateOffset(years=YRS_AFTER)
        # per-ticker source with fallback: try primary, then the other source if empty
        s, used = None, SOURCE
        try:
            s = get_prices(ticker, full_start, full_end)
        except Exception:
            s = None
        if (s is None or len(s) < 2):   # fallback to the other free source
            try:
                alt = "stooq" if SOURCE == "yfinance" else "yfinance"
                s2 = prices_stooq(ticker, full_start, full_end) if alt == "stooq" \
                     else prices_yf(ticker, full_start, full_end)
        ...
        if s is None or len(s) < 2:
            rows.append({"transition_id": tid, "coname": p["coname"], "ticker": ticker,
                         "status": "no-price-data"}); continue

        def ret_between(series, a, b):
            seg = series.loc[a:b]
            return window_return(seg)
        def mkt_between(a, b):
            return window_return(mkt.loc[a:b]) if mkt is not None else None

        # three event-time windows: pre (-2->0), post (0->+3), full (-2->+3)
        tsr_full = ret_between(s, full_start, full_end)
        tsr_pre  = ret_between(s, full_start, appt)       # before the hire (selection)
        tsr_post = ret_between(s, appt, full_end)         # AFTER the hire (event-time)
        mk_full, mk_pre, mk_post = mkt_between(full_start, full_end), mkt_between(full_start, appt), mkt_between(appt, full_end)
        def adj(r, mk): return (r - mk) if (r is not None and mk is not None) else None
        rows.append({
            "transition_id": tid, "coname": p["coname"], "ticker": ticker,
            "classification": p["classification"], "price_source": used,
            "window_start": full_start.date(), "window_end": full_end.date(),
            "tsr_raw": tsr_full, "mkt_return": mk_full, "tsr_mktadj": adj(tsr_full, mk_full),
            "tsr_pre_raw": tsr_pre, "tsr_pre_mktadj": adj(tsr_pre, mk_pre),
            "tsr_post_raw": tsr_post, "tsr_post_mktadj": adj(tsr_post, mk_post),  # post-hire-only
            "status": "ok" if tsr_full is not None else "no-price-data",
        })
